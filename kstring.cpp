...
kstring& kstring::operator+=(const kstring& other) { return Append(other); }
...
{
    auto str = static_cast<wchar_t*>(
        ExAllocatePoolWithTag(m_Pool, sizeof(WCHAR) * (chars + 1), m_Tag));
    if (!str)
    {
    ...
    if (src)
    {
        ...
        memcpy_s(str, sizeof(WCHAR) * (chars + 1), src,
                 sizeof(WCHAR) * (chars + 1));
    }
    return str;
}
