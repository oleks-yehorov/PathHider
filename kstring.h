...
{
  public:
    explicit kstring(const wchar_t* str = nullptr,
    ...
    explicit kstring(PCUNICODE_STRING str,
                     POOL_TYPE pool = PagedPool,
                     ULONG tag = 0);
    ...
    const wchar_t* Get() const { return m_str; }
    ...
    const wchar_t operator[](size_t index) const { return GetAt(index); }

    wchar_t& operator[](size_t index) { return GetAt(index); }
